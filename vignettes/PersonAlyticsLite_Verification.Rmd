---
title: "Verification Tests for PersonAlyticsLite"
...
## Introduction
The pupropose of this vignette is to document unit testing of the `PersonAlyticsLite` software. In what follows, the test `all.equal()` is used to verify that output is equivalent to expected results. If the test passes, the result printed out will be `TRUE`. When this document is produced using the current version of `PersonAlyticsLite`, the file `"YourRLibrary\PersonAlyticsLite\inst\doc\PersonAlyticsLite_Verification.R"` is created with all the `R` code in this document. The file `PersonAlyticsLite_Verification.R` can be run to verify results using the installed version of `PersonAlyticsLite`.

First, load `PersonAlyticsLite`:
...
Create a new `Palytic` (see `?Palytic` after loading `PersonAlyticsLite`) object using the OvaryICT data (see `?OvaryICT` after loading `PersonAlyticsLite`).
...
Since a `Palytic` object can be used to fit models using both `lme` and `gamlss`, formula updating is implemented via `R6` active bindings. First we create a new `Palytic` object using the OvaryICT data (see `?OvaryICT` after loading `PersonAlyticsLite`).
