                          ...
                          formula=NULL)            )
{
  ...
  for(dv in dims$DV)
  {
    ...
    for(i in 1:nrow(DIM))
    {
      ...
      # add the target IV
      cat(toString(t1$formula), file="t1formula.txt",append=TRUE)
      ...
      cat("I also made it to line 435", file="test.txt")
      cat(is(err_id), file="test.txt", append=TRUE)
      save(err_id, Model, file="line435.Rdata")

      IDout[[i]] <- # this line stays commented  out except for testing
        return( list( Messages=err_id, Model=Model ) )
      cat("I returned at line 438", file="test.txt", append=TRUE)
      ...
      cat("I repopulated t0 at line 444", file="test.txt", append=TRUE)

      # clean up
      rm(t1)
      cat('\n\n')
      cat("I cleaned up at line 448", file="test.txt", append=TRUE)
